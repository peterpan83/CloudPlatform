...
class CloudPlatform():
    ...
    def reset(self):
        ...
        if self.currentHorizon-self.AzimuthSensorIni >= 180:
            Cprotocol.pan_speed = 30
            # 先左转180度,为了防止总是往一个方向转，线发生线缠绕的情况
            secs = 180 / (30 * 0.15)
            self.protocolD.left(secs)
            # print('左转了%d秒'%(secs))
    ...
    def init(self):
        ...
        if abs(self.AzimuthSensorIni) > 10:
            ...
            Cprotocol.pan_speed = self.speed
        secs = abs(self.AzimuthSensorIni)/(Cprotocol.pan_speed*0.15)
        if self.AzimuthSensorIni > 0:
            self.protocolD.left(secs)
        else:
            self.protocolD.right(secs)
    ...
    def first_adjust(self):
        ...
        print(now)
        if AzimuthSensor < self.AzimuthSensorMin or AzimuthSensor>self.AzimuthSensorMax:
            ...
            secs = AzimuthSensor / (Cprotocol.pan_speed * 0.15)  ##当前平台的方位角与太阳的方位角之差，在当前速度下所需时间

            print('当前HyperSAS方位：%.2f,太阳方位：%.2f,角度差：%.2f,按当前速度%.3f/s,需旋转%.3f秒。'
                  % (self.currentHorizon, sa, AzimuthSensor, Cprotocol.pan_speed * 0.15, secs))
            self.protocolD.right(secs)
    ...
    def auto(self):
        ...
        if delta > 10:
            ...
            Cprotocol.pan_speed = self.speed
        secs = delta/(Cprotocol.pan_speed*0.15) ##当前平台的方位角与太阳的方位角之差，在当前速度下所需时间
        # print(secs)
        print('当前HyperSAS方位：%.2f,太阳方位：%.2f,角度差：%.2f,按当前速度%.3f/s,需旋转%.3f秒。'
              % (self.currentHorizon, sa, delta, Cprotocol.pan_speed*0.15, secs))
        self.protocolD.right(secs)
        # print('旋转完成')
        self.currentHorizon = AzimuthSensor
...
if __name__ == '__main__':
    ...
    pCloudPlatform.setSpeed(v_speed)
